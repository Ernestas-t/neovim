...
vim.keymap.set("n", "<S-Tab>", ":BufferLineCyclePrev<CR>", { desc = "Next previous buffer" })
vim.keymap.set("n", "<leader>x", ":Bdelete<CR>", { desc = "E[x]it Buffer" })

-- tmuxnavigator mappings
...
require("lazy").setup({
	...
	"christoomey/vim-tmux-navigator", --add vim motion navigation between neovim and tmux
		...
		config = function()
			require("bufferline").setup({
				options = {
					...
					-- Show icons for modified buffers and close button
					modified_icon = "●",
					close_icon = "",
					...
					-- Truncate long names and set max length
					truncate_names = true,
					max_name_length = 18,
